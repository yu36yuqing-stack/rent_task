...
后续 H5 页面开发，按钮与头部筛选必须遵循统一分层，跨页面保持一致：

- 页面级按钮：
  - 统一使用 `.btn-page-action`。
  - 适用场景：页面头部右上角主操作（如同步/刷新）。
- 卡片级按钮：
  - 统一使用 `.btn-card-action`。
  - 适用场景：卡片内部操作（如状态查询、更多操作）。
- 页面头部第一级筛选：
  - 统一使用 `.header-tab`（基于 `.orders-tab`）。
- 页面头部第二级快捷筛选：
  - 统一使用 `.header-quick-tab`（基于 `.orders-quick-item`）。
- 页面头部第三级信息汇总文案：
  - 统一使用 `.head-summary-text`。

实施要求：

1. 新页面优先复用上述类，不允许为同义场景新增另一套样式。
2. 页面专属差异必须通过页面锚点变体实现，禁止直接覆盖全局基线。
3. 若确需扩展，必须同步更新 `h5/COMPONENT_GUIDE.md` 记录变更。
