...
| Pricing | `.pricing-ladder-head` `.pricing-ladder-list` `.pricing-account-card` `.pricing-ladder-grid` `.pricing-copy-field` `.pricing-search-field` `.pricing-channel-tabs` `.pricing-package-table` `.pricing-ratio-panel` `.pricing-ratio-grid` `.pricing-ratio-example` `.pricing-ratio-result` | 基于 GameTabs、Panel、Card、Field、ListSearch、CardAction、PageAction、HeaderTabs、Sheet；四档统一时租输入、账号复制、当前游戏内搜索、渠道动态套餐列和按渠道套餐倍率表单为页面变体。套餐倍率页直接复用 HeaderTabs、Panel、Field 与 PageAction，头部示例时租价只用于前端演算、不持久化，结果以普通文字紧跟各倍率字段展示；不在通用账号卡片写入渠道字段 |
