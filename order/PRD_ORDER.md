...
- 触发条件：
  ...
  2. 当前时间已经进入“临近结束窗口”：`now >= order_end_time - near_end_sec`。
  3. 当前时间尚未超过冷却释放截止：`now < order_end_time + release_delay_min`。
- 默认参数：
  1. `near_end_sec = 30 秒`。
  2. 即默认在“订单结束前 30 秒”开始扫描命中；若任务错过该窗口，订单结束后但未到释放截止前仍可补建冷却期。
- 入黑名单动作：
  1. 写入/更新 `reason=冷却期下架`。
  2. 计算并保存 `cooldown_until = order_end_time + release_delay_min`。
- 说明：
  1. 这里不是“订单开始后 10 分钟触发”。
  2. 冷却截止时间始终按订单结束时间计算，不按任务实际执行时间顺延。
...
- 已确认：
  ...
  4. 入场时机以当前代码为准：结束前 `near_end_sec` 到释放截止前命中，而非开始后 10 分钟。
...
- 风险点：
  ...
  2. N 单下架没有复用冷却期“结束前 30 秒到释放截止前扫描”的更早触发时机。

### 11. N 单下架与冷却期下架收口落地方案
- 目标：
  1. 统一订单类自动下架的判定入口。
  2. 让 N 单下架复用冷却期下架“结束前 30 秒到释放截止前扫描”的触发时机。
...
- 预期效果：
  1. 第 3 单进入结束前 30 秒到释放截止前窗口时，若已满足 N 单阈值，可进入 `order_n_off`。
  2. 正常情况下，账号可在第 3 单结束前完成下架，降低出现第 4 单的概率。
- 边界说明：
  1. 若平台订单同步延迟，导致第 3 单在结束前 30 秒到释放截止前窗口内仍未入库，则仍可能漏过提前命中。
