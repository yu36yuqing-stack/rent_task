...
async function main() {
    ...
    assertEqual(Number((built.info || {}).rentalByNight || 0), 15.2, 'build payload preserves current night price');
    assertEqual(Number((built.info || {}).rentalByDay || 0), 22.8, 'build payload preserves current day price');
    assertEqual(Number((built.info || {}).rentalByWeek || 0), 152, 'build payload preserves current week price');
    ...
    try {
        ...
        assertEqual(Number((((priceReq || {}).info || {}).rentalByNight || 0)), 18.8, 'price api keeps explicit night price');
        assertEqual(Number((((priceReq || {}).info || {}).rentalByDay || 0)), 22.8, 'price api preserves current day price');
        assertEqual(Number((((priceReq || {}).info || {}).rentalByWeek || 0)), 152, 'price api preserves current week price');
