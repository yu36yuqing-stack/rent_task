...
- 规则：
  - 不提供管理员分配归属能力
  - 不提供用户手工改账号能力
  - 同一个游戏账号可在多个平台同时存在，按“账号 + 平台”维度记录状态
- 最小接口：
...
- 字段：
  ...
  - `has_uhaozu`（0/1）
  - `has_youyouzuhao`（0/1）
  - `zuhaowang_status`（上架/下架/租赁中/未知）
  - `uhaozu_status`（上架/下架/租赁中/审核失败/未知）
  - `youyouzuhao_status`（上架/下架/租赁中/未知）
  - `modify_date`
  - `is_deleted`
  - `desc`
...
### 11.2 待确认问题
1. 一个账号是否允许多用户共享？（建议默认不允许）
2. 平台授权过期后的通知重试策略（频率/次数）？
3. 通知路由失败时是否回退给管理员？
4. 多游戏字段是否现在就强制必填？
